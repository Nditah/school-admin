...
import { AccountClass, Staff, SelectOption, ApiResponse } from '../../models';
...
export class AccountClassComponent implements OnInit {
  ...
  page_name = 'List of AccountClasses';
  currentRecords: Array<AccountClass>;
  loading: false;
  ...
  currentRecord: AccountClass;

  constructor(public accountClasses: AccountClasses,
              private notify: NotificationService,
              private _fb: FormBuilder,
              private staffs: Staffs) {
                this.currentRecords = this.accountClasses.query();
   ...
   */
  openSidebar(activePanel: string, status: string, record: AccountClass | null) {
    this.sidebarView = activePanel;
    this.sidebarContent = status;
    this.activeSidebar = true;
    this.sidebarHeading = `${status.replace(/^[a-zA-Z]/, (c) => c.toUpperCase())} AccountClass`;
  ...
  onChange(event: any, name: string) {
    ...
    this.accountClasses.recordRetrieve('?code=' + code).then((data: ApiResponse) => {
      if (data.success) {
  ...
  async returnResponse(event: any) {
    ...
    const results = await this.accountClasses.recordRetrieve();
    if (results.success) {
