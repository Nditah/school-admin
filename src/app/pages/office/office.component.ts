...
import { Office, Staff, SelectOption, ApiResponse } from '../../models';
...
export class OfficeComponent implements OnInit {
  ...
  page_name = 'List of Offices';
  currentRecords: Array<Office>;
  loading: false;
  ...
  currentRecord: Office;

  constructor(public offices: Offices,
              private notify: NotificationService,
              private _fb: FormBuilder,
              private staffs: Staffs) {
                this.currentRecords = this.offices.query();
   ...
   */
  openSidebar(activePanel: string, status: string, record: Office | null) {
    this.sidebarView = activePanel;
    this.sidebarContent = status;
    this.activeSidebar = true;
    this.sidebarHeading = `${status.replace(/^[a-zA-Z]/, (c) => c.toUpperCase())} Office`;
  ...
  onChange(event: any, name: string) {
    ...
    this.offices.recordRetrieve('?code=' + code).then((data: ApiResponse) => {
      if (data.success) {
  ...
  async returnResponse(event: any) {
    ...
    const results = await this.offices.recordRetrieve();
    if (results.success) {
